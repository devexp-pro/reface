...
JSX provides a familiar syntax for NodeProxy creation, but it's just syntactic sugar over NodeProxy API calls.
...
JSX syntax is transformed into NodeProxy API calls:
...
// Compiles to NodeProxy API calls
...
5. Children are handled via template literals
