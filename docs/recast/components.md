...
Component System allows creating reusable elements with unique IDs and client-side functionality through ComponentNodeProxy.
...
const Card = component((props) => (
  <div __rcc={props.__rcc}>
    {" "}
    {/* Component ID is passed via props */}
    <h2>{props.title}</h2>
...
const Button = component((props) => (
  <div>
    ...
    <script data-component={props.__rcc}>{script}</script>
  </div>
));
...
```typescript
// Regular function returns NodeProxy
...
// Component returns ComponentNodeProxy
